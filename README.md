...
프론트엔드는 배포 환경에서 기본 API 주소로 `https://sql-visual.onrender.com/api`를 사용하도록 설정되어 있습니다. 이 주소로 백엔드를 Render에 배포하고 네이버 Developers Callback URL을 맞추면 다른 컴퓨터에서도 로그인, 내 문서, 공유 게시판, 댓글 기능을 사용할 수 있습니다. 내 문서와 공유 게시판 데이터가 배포 후 초기화되지 않게 하려면 Render에 영구 디스크를 붙이고 `DATA_FILE=/var/data/sqlvisual-data.json`을 설정하세요.
